...
local YOU_RECEIVE_PATTERNS = {"你获得了物品：", "你获得了", "获得物品", "你得到了", "你拾取了", "你制造了", "你赢得了"}
local BRACKET_OPEN = "%["
local BRACKET_CLOSE = "%]"
...
function LootMonitor:ProcessCoinMessage(message)
        ...
        if goldStr and silverStr then
            ...
            if goldStr and copperStr then
                ...
                if silverStr and copperStr then
                    ...
                    if goldStr then goldAmount = tonumber(goldStr) or 0 end
                    pattern = "(%d+)[%s,]*银"
                    _, _, silverStr = strfind(message, pattern)
                    if silverStr then silverAmount = tonumber(silverStr) or 0 end
                    pattern = "(%d+)[%s,]*铜"
                    _, _, copperStr = strfind(message, pattern)
                    if copperStr then copperAmount = tonumber(copperStr) or 0 end
                end
            end
        end
    end
    ...
    if existingCoinNotification then
        ...
        local coinText = ""
        if totalGold > 0 then coinText = coinText .. totalGold .. "金" end
        if totalSilver > 0 then coinText = coinText .. totalSilver .. "银" end
        if totalCopper > 0 then coinText = coinText .. totalCopper .. "铜" end
...
function LootMonitor:ProcessLootMessageCN(message)
    ...
    if self:IsCoinMessageCN(message) then self:ProcessCoinMessage(message) return end
    local isReceiveMessage = false
    for i = 1, tgetn(YOU_RECEIVE_PATTERNS) do
        if strfind(message, YOU_RECEIVE_PATTERNS[i]) then isReceiveMessage = true break end
    end
...
function LootMonitor:ProcessSystemMessageCN(message)
    if not message or not LootMonitorDB.enabled then return end
    if self:IsCoinMessageCN(message) then self:ProcessCoinMessage(message) return end
    if strfind(message, "获得物品") or strfind(message, "你获得了") or strfind(message, "你得到了") or 
       strfind(message, "你拾取了") or strfind(message, "你制造了") or strfind(message, "获得") or 
       strfind(message, "你赢得了") then
...
function LootMonitor:AddLootItem(itemData, isNameOnly, quantity, isCoin, gold, silver, copper)
    ...
    if not isNameOnly then
        ...
        if bracketStart and bracketEnd then itemName = strsub(itemData, bracketStart + 1, bracketEnd - 1) else itemName = "未知物品" end
    else itemName = itemData end
...
function LootMonitor:OnLoad()
    if not LootMonitorDB then LootMonitorDB = {} end
    for key, value in pairs(defaults) do
        if LootMonitorDB[key] == nil then
            if key == "position" then LootMonitorDB[key] = { point = value.point, x = value.x, y = value.y } else LootMonitorDB[key] = value end
...
function LootMonitor:SavePosition()
    if self.frame then
        ...
        if point and x and y then
            if not LootMonitorDB.position then LootMonitorDB.position = {} end
...
function LootMonitor:RegisterEvents()
    ...
    frame:SetScript("OnEvent", function()
        if event == "ADDON_LOADED" then
            if not LootMonitor.initialized then LootMonitor:OnLoad() LootMonitor.initialized = true end
        elseif event == "CHAT_MSG_LOOT" then LootMonitor:ProcessLootMessageCN(arg1)
        elseif event == "CHAT_MSG_MONEY" then LootMonitor:ProcessCoinMessage(arg1)
        elseif event == "CHAT_MSG_SYSTEM" then LootMonitor:ProcessSystemMessageCN(arg1)
        elseif event == "PLAYER_LOGOUT" then LootMonitor:SavePosition() end
    end)
end
                ...
                if itemLink then
                    ...
                    if linkStart and linkEnd then
                        ...
                        if linkName == itemName then
                            ...
                            if texture then return texture, bag, slot end
                        end
                    end
                end
            end
                ...
                if itemLink then
                    ...
                    if linkStart and linkEnd then
                        ...
                        if linkName == itemName then
                            ...
                            if itemCount then totalCount = totalCount + itemCount end
                        end
                    end
                end
            end
...
function LootMonitor:CleanupNotifications()
    ...
    for i, notification in ipairs(self.activeNotifications) do
        ...
        if elapsed > totalTime then tinsert(toRemove, notification) end
    end
    for _, notification in ipairs(toRemove) do self:RemoveNotification(notification) end
end

function LootMonitor:RemoveNotification(notification)
    if notification.animFrame then notification.animFrame:SetScript("OnUpdate", nil) notification.animFrame = nil end
    if notification.bgAnimFrame then notification.bgAnimFrame:SetScript("OnUpdate", nil) notification.bgAnimFrame = nil end
    if notification.frame then notification.frame:Hide() notification.frame:SetParent(nil) notification.frame = nil end
    local activeList = self.activeNotifications
    for i = 1, tgetn(activeList) do
        if activeList[i] == notification then tremove(activeList, i) break end
    end
    self:RepositionNotifications()
end
...
function LootMonitor:UpdateNotificationText(notification)
    ...
    if notification.count > 1 then displayText = displayText .. " x" .. notification.count end
    ...
    local showTotal = LootMonitorDB.showTotalCount and not notification.isCoin and self:CountItemInBags(notification.name) > 0
    local iconY = -24
    local textYOffset
    if showTotal then
        textYOffset = 5
    ...
    searchFrame:SetScript("OnUpdate", function()
        ...
        if elapsed > maxSearchTime then
            if not fallbackUsed then
                ...
                if fallbackTexture then notification.icon:SetTexture(fallbackTexture) end
            end
            searchFrame:SetScript("OnUpdate", nil)
            return
        end
        if timeSinceLastSearch < searchInterval then return end
        lastSearch = gettime()
...
function LootMonitor:GetFallbackIcon(itemName)
    ...
    local name = strlower(itemName)
    if strfind(name, "铜") then return COIN_ICON_COPPER
    elseif strfind(name, "银") then return COIN_ICON_SILVER
    elseif strfind(name, "金") then return COIN_ICON_GOLD
    elseif strfind(name, "药") then return "Interface\\Icons\\INV_Potion_52"
    elseif strfind(name, "布") then return "Interface\\Icons\\INV_Fabric_Linen_01"
    elseif strfind(name, "皮") then return "Interface\\Icons\\INV_Misc_LeatherScrap_02"
    elseif strfind(name, "矿") then return "Interface\\Icons\\INV_Ore_Copper_01"
    elseif strfind(name, "草") or strfind(name, "花") then return "Interface\\Icons\\INV_Misc_Herb_07"
    elseif strfind(name, "石") then return "Interface\\Icons\\INV_Misc_Gem_01"
    elseif strfind(name, "食物") or strfind(name, "面包") or strfind(name, "肉") then return "Interface\\Icons\\INV_Misc_Food_15"
    else return TEXTURE_PATH_QUESTION end
end
...
function LootMonitor:StartNotificationAnimation(notification)
    ...
    animFrame:SetScript("OnUpdate", function()
        ...
        elseif elapsed < fadeInTime + displayTime + fadeOutTime then
            if not notification.fadingOut then notification.fadingOut = true end
            ...
            notification.frame:SetAlpha(alpha)
            notification.frame:SetScale(scale)
        else self:RemoveNotification(notification) end
    end)
end
...
function LootMonitor:CreateLootNotification(itemName, quantity, itemData, isNameOnly, isCoin, gold, silver, copper)
    ...
    if isCoin then
        ...
        if coinTexture then icon:SetTexture(coinTexture) else icon:SetTexture(COIN_ICON_GOLD) end
    else
...
function LootMonitor:CreateSettingsPanel()
    ...
    frame:SetBackdrop({ edgeFile = "Interface\\DialogFrame\\UI-DialogBox-Border", tile = true, tileSize = 32, edgeSize = 32, insets = { left = 12, right = 12, top = 12, bottom = 12 } })
    ...
    enableCheck:SetScript("OnClick", function() LootMonitorDB.enabled = enableCheck:GetChecked() end)
    ...
    totalCountCheck:SetScript("OnClick", function() LootMonitorDB.showTotalCount = totalCountCheck:GetChecked() end)
    ...
    moveBtn:SetScript("OnClick", function()
        ...
        if LootMonitor.moveMode then moveBtn:SetText("退出移动") else moveBtn:SetText("移动位置") end
    end)
...
function LootMonitor:ShowSettings()
    if not self.settingsFrame then self:CreateSettingsPanel() end
...
function LootMonitor:EnterMoveMode()
    ...
    if not self.moveFrame then
        ...
        if mainPoint and mainX and mainY then moveFrame:SetPoint(mainPoint, UIParent, mainPoint, mainX, mainY)
        else moveFrame:SetPoint("CENTER", UIParent, "CENTER", 200, 100) end
        ...
        border:SetBackdrop({ edgeFile = "Interface\\DialogFrame\\UI-DialogBox-Border", edgeSize = 16, insets = { left = 4, right = 4, top = 4, bottom = 4 } })
        ...
        moveFrame:SetScript("OnDragStart", function() moveFrame:StartMoving() end)
...
SlashCmdList["LOOTMONITOR"] = function(msg)
    ...
    if cmd == "toggle" then
        ...
        if LootMonitorDB.enabled then Print("战利品通知已启用") else Print("战利品通知已禁用") end
    elseif cmd == "" then LootMonitor:ShowSettings()
    elseif cmd == "clear" then
        for _, notification in ipairs(LootMonitor.activeNotifications) do LootMonitor:RemoveNotification(notification) end
        Print("已清除所有通知")
    elseif cmd == "move" then LootMonitor:ToggleMoveMode()
    elseif cmd == "settings" then LootMonitor:ShowSettings()
    elseif cmd == "help" then
        ...
        Print("  /lm help - 显示帮助")
    else Print("未知命令，输入 /lm help 查看帮助") end
end
